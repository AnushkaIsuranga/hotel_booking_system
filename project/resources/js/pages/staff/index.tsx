import { ConfirmAction } from '@/components/hotel/confirm-action';
import { PageHeader } from '@/components/hotel/page-header';
import { StatusBadge } from '@/components/hotel/status-badge';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import AppLayout from '@/layouts/app-layout';
import { type BreadcrumbItem } from '@/types';
import { Head, Link, router } from '@inertiajs/react';
import { Edit3, Trash2 } from 'lucide-react';

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Dashboard', href: '/dashboard' },
    { title: 'Team', href: '/staff' },
];

interface StaffRow {
    id: number;
    name: string;
    email: string;
    role: 'admin' | 'staff';
    job_title: string | null;
    phone: string | null;
    is_active: boolean;
    last_login_at: string | null;
}

interface StaffIndexProps {
    users: StaffRow[];
    summary: {
        active_staff: number;
        admins: number;
        front_office: number;
    };
    currentUserId: number | null;
}

export default function StaffIndex({ users, summary, currentUserId }: StaffIndexProps) {
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Team" />

            <div className="min-h-full space-y-6 bg-[radial-gradient(circle_at_top_left,_rgba(204,181,126,0.14),_transparent_28%),linear-gradient(180deg,_#f7f3ea_0%,_#f4f8f7_48%,_#ffffff_100%)] p-4 dark:bg-[radial-gradient(circle_at_top_left,_rgba(204,181,126,0.08),_transparent_28%),linear-gradient(180deg,_#102925_0%,_#0f2622_55%,_#0b1d1a_100%)] md:p-6">
                <PageHeader
                    eyebrow="Access Control"
                    title="Hotel Team"
                    description="Control staff and administrator access for the hotel operations workspace."
                    action={{ href: route('staff.create'), label: 'Add Team Member' }}
                />

                <div className="grid gap-4 md:grid-cols-3">
                    {[
                        { label: 'Active Accounts', value: summary.active_staff },
                        { label: 'Administrators', value: summary.admins },
                        { label: 'Staff Accounts', value: summary.front_office },
                    ].map((item) => (
                        <Card key={item.label} className="rounded-[24px] border-0 bg-white shadow-[0_18px_40px_-28px_rgba(14,49,44,0.35)] dark:border dark:border-[#2a4f47] dark:bg-[#14312c]/95 dark:shadow-[0_18px_40px_-28px_rgba(0,0,0,0.8)]">
                            <CardContent className="space-y-2 p-5">
                                <p className="text-xs tracking-[0.2em] text-[#8a6a2f] uppercase dark:text-[#cfb67c]">{item.label}</p>
                                <p className="font-display text-3xl text-[#14312c] dark:text-[#f5eed9]">{item.value}</p>
                            </CardContent>
                        </Card>
                    ))}
                </div>

                <Card className="rounded-[28px] border-white/80 bg-white/95 shadow-[0_20px_50px_-32px_rgba(14,49,44,0.45)] dark:border-[#294941] dark:bg-[#14312c]/95 dark:shadow-[0_20px_50px_-32px_rgba(0,0,0,0.8)]">
                    <CardContent className="overflow-hidden rounded-[24px] border border-[#e4ece8] p-0 dark:border-[#2a4f47]">
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-[#f8faf9] dark:bg-[#173a33]">
                                    <TableHead>Team Member</TableHead>
                                    <TableHead>Role</TableHead>
                                    <TableHead>Contact</TableHead>
                                    <TableHead>Status</TableHead>
                                    <TableHead>Last Login</TableHead>
                                    <TableHead className="text-right">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {users.map((user) => (
                                    <TableRow key={user.id}>
                                        <TableCell>
                                            <div className="space-y-1">
                                                <p className="font-medium text-[#14312c] dark:text-[#f5eed9]">{user.name}</p>
                                                <p className="text-muted-foreground text-sm">{user.job_title || 'Hotel team member'}</p>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <StatusBadge value={user.role} variant="role" />
                                        </TableCell>
                                        <TableCell className="text-muted-foreground text-sm">
                                            <p>{user.email}</p>
                                            <p>{user.phone || 'Phone not set'}</p>
                                        </TableCell>
                                        <TableCell>
                                            <span
                                                className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${
                                                    user.is_active
                                                        ? 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800/70 dark:bg-emerald-900/40 dark:text-emerald-300'
                                                        : 'border-stone-200 bg-stone-100 text-stone-700 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300'
                                                }`}
                                            >
                                                {user.is_active ? 'Active' : 'Inactive'}
                                            </span>
                                        </TableCell>
                                        <TableCell className="text-muted-foreground text-sm">{user.last_login_at || 'Not signed in yet'}</TableCell>
                                        <TableCell>
                                            <div className="flex justify-end gap-2">
                                                <Button variant="outline" size="sm" asChild className="rounded-full">
                                                    <Link href={route('staff.edit', user.id)}>
                                                        <Edit3 className="mr-2 h-4 w-4" />
                                                        Edit
                                                    </Link>
                                                </Button>
                                                {user.id !== currentUserId && (
                                                    <ConfirmAction
                                                        title="Remove team member?"
                                                        description="This will permanently remove the staff account from the hotel operations system."
                                                        actionLabel="Remove account"
                                                        destructive={true}
                                                        onConfirm={() => router.delete(route('staff.destroy', user.id))}
                                                    >
                                                        <Button
                                                            variant="outline"
                                                            size="sm"
                                                            className="rounded-full text-rose-600 hover:text-rose-700"
                                                        >
                                                            <Trash2 className="mr-2 h-4 w-4" />
                                                            Remove
                                                        </Button>
                                                    </ConfirmAction>
                                                )}
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
                    </CardContent>
                </Card>
            </div>
        </AppLayout>
    );
}
